"""
Claude API Service สำหรับระบบช่วยวินิจฉัยโรครากฟัน
ให้คำแนะนำการรักษาและตอบคำถามแพทย์
"""
import os
import json
import base64
...
class ClaudeService:
    def __init__(self):
        ...
        self.medical_system_prompt = """คุณเป็นระบบ AI ช่วยสนับสนุนการตัดสินใจของแพทย์ทันตกรรม โดยมีหน้าที่ให้คำแนะนำทางการแพทย์อ้างอิงหลักฐานทางวิทยาศาสตร์
...
- ระดับความเชื่อมั่น: {confidence:.1f}%"""
    ...
    def chat_with_doctor(
        ...
        chat_system = """คุณเป็น AI ผู้ช่วยแพทย์ทันตกรรมที่เป็นมิตรและช่วยเหลือ โดยมีหน้าที่ตอบคำถามและให้คำแนะนำเบื้องต้น
...
- หากมีข้อมูลบริบทการตรวจพบโรค สามารถอ้างอิงได้"""
    ...
    def validate_dental_xray(self, image_bytes: bytes) -> Dict[str, Any]:
        """
        ตรวจสอบว่าภาพที่อัปโหลดเป็น X-ray ฟันหรือไม่
...
1. ภาพ X-ray ทางทันตกรรม (Dental X-ray) หรือไม่
2. มีฟันหรือโครงสร้างในช่องปากปรากฏหรือไม่
3. เป็นภาพทางการแพทย์ที่เกี่ยวข้องกับทันตกรรมหรือไม่
...
หากเป็น X-ray ฟัน: is_valid = true
หากไม่ใช่ X-ray ฟัน: is_valid = false"""
                    }
                ]
            }
            ...
            try:
                ...
                return {
                    "is_valid": False,
                    "confidence": 0,
                    "message": "ไม่สามารถวิเคราะห์ภาพได้ กรุณาอัปโหลดภาพ X-ray ฟันที่ชัดเจน"
                }
